...
@section('content')
                        ...
                        <div class="row align-items-center">
                            <div class="col">
                                <div class="breadcrumbs-area clearfix">
                                    <h4 class="page-title pull-left">{{\Session::get('p_loc_name')}} > Settings > Vessel > Edit</h4>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="row">
        <div class="col-md-6 col-sm-12 mt-2">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Edit Vessel</h4>
                    @include('notifications')
                    <form action="{{ route('settings.vessel.update') }}" method="POST">
                        @csrf
                        <input hidden name="id" value="{{$vessel->id}}">
                        <div class="form-group">
                            <label class="col-form-label">Primary Location</label>
                            <select id="plocation_id" name="plocation_id" class="custom-select">
                                @foreach($locations as $item)
                                    <option {{$vessel->plocation_id==$item->id?'selected':''}} value="{{$item->id}}">{{$item->location}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="vessel" class="col-form-label">Vessel</label>
                            <input class="form-control" value="{{$vessel->vessel}}" type="text" name="vessel" id="vessel">
                        </div>
                        <div class="form-group">
                            <label for="location" class="col-form-label">Location Name</label>
                            <input class="form-control"  value="{{$vessel->location_name}}" type="text" name="location" id="location">
                        </div>
                        <div class="form-group" id="color-group">
                            <label for="location_code" class="col-form-label">Location Code</label>
                            <input class="form-control"  value="{{$vessel->location_code}}" type="text" name="location_code" id="location_code">
                        </div>
                        <div class="form-group">
                            <label for="location_latitude" class="col-form-label">Location Latitude</label>
                            <input class="form-control"  value="{{$vessel->location_latitude}}" type="text" name="location_latitude" id="location_latitude" >
                        </div>
                        <div class="form-group">
                            <label for="location_longitude" class="col-form-label">Location Longitude</label>
                            <input class="form-control" value="{{$vessel->location_longitude}}" type="text" name="location_longitude" id="location_longitude" >
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input id="last_inspected" class="form-control" type="date" value="{{date('Y-m-d',strtotime($vessel->last_inspected))}}" name="last_inspected">
                        </div>
                        <div class="form-group">
                            <label for="vessel_rate" class="col-form-label">VESSEL MAX RATED FLOW RATE LITERS/MIN</label>
                            <input class="form-control" value="{{$vessel->vessel_rate}}" type="number" name="vessel_rate" id="vessel_rate" >
                        </div>
                        <div class="form-group">
                            <label for="filter_type" class="col-form-label">FILTER TYPE/MANUFACTURE</label>
                            <input class="form-control" value="{{$vessel->filter_type}}" type="text" name="filter_type" id="filter_type" >
                        </div>

                        <div class="form-group">
                            <label for="filter_serial" class="col-form-label">ELEMENT TYPE</label>
                            <input class="form-control"  value="{{$vessel->filter_serial}}" type="text" name="filter_serial" id="filter_serial" >
                        </div>

                        <div class="form-group">
                            <label for="qty" class="col-form-label">QTY OF FILTERS INSTALLED</label>
                            <input class="form-control"  value="{{$vessel->qty}}" type="number" name="qty" id="qty" >
                        </div>
                        <div class="form-group">
                            <div class="panel-body">
                                ...
                                <div class="dropzone mb-3" id="settings_images">
                                    @if($vessel->images)
                                        @if($images = json_decode($vessel->images))
                                            ...
                                            @endforeach
                                        @else
                                            <div class="dz-preview dz-image-preview" data-img="{{$vessel->images}}">
                                                <div class="dz-image">
                                                    <img src="{{asset('uploads/settings/'.$vessel->images)}}" style="width: 120px;height: 120px" />
                                                </div>
                                                <div class="dz-details">
                                                    <div class="dz-filename"><span data-dz-name="">{{$vessel->images}}</span></div>
                                                </div>
                                                <a class="dz-remove" href="javascript:;" onclick="remove_files('{{$vessel->images}}','images')" data-dz-remove="">Remove Image</a>
                                            </div>
                                        @endif
                        ...
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->water_defense==1?'checked="checked"':''}} class="custom-control-input" name="water_defense" id="water_defense">
                            <label class="custom-control-label" for="water_defense">Water Defense System</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->vessel_filter==1?'checked="checked"':''}} class="custom-control-input" name="vessel_filter" id="vessel_filter">
                            <label class="custom-control-label" for="vessel_filter">Vessel Filter Sump</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->bonding_cable==1?'checked="checked"':''}} class="custom-control-input" name="bonding_cable" id="bonding_cable">
                            <label class="custom-control-label" for="bonding_cable">Bonding Cable, Scully System Continuity Test Inspection</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->differential_pressure==1?'checked="checked"':''}} class="custom-control-input" name="differential_pressure" id="differential_pressure">
                        ...
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->filter_membrane==1?'checked="checked"':''}} class="custom-control-input" name="filter_membrane" id="filter_membrane">
                            <label class="custom-control-label" for="filter_membrane">Filter Membrane Test(Millipore)</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->deadman_control==1?'checked="checked"':''}} class="custom-control-input" name="deadman_control" id="deadman_control">
                            <label class="custom-control-label" for="deadman_control">Deadman Control Check</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->hoses_pumps_screens==1?'checked="checked"':''}} class="custom-control-input" name="hoses_pumps_screens" id="hoses_pumps_screens">
                            <label class="custom-control-label" for="hoses_pumps_screens">Hoses, Pumps and Screens</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->bol==1?'checked="checked"':''}} class="custom-control-input" name="bol" id="bol">
                            <label class="custom-control-label" for="bol">Truck - Bill of lading</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->bol_pipeline==1?'checked="checked"':''}} class="custom-control-input" name="bol_pipeline" id="bol_pipeline">
                            <label class="custom-control-label" for="bol_pipeline">Pipeline - Bill of lading</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->eye_wash_inspection==1?'checked="checked"':''}} class="custom-control-input" name="eye_wash_inspection" id="eye_wash_inspection">
                            <label class="custom-control-label" for="eye_wash_inspection">Eye Wash Inspection</label>
                        </div>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" {{$vessel->bulk_sump==1?'checked="checked"':''}} checked="checked" class="custom-control-input" name="bulk_sump" id="bulk_sump">
...
@section('footer_scripts')
    ...
    <script>
        ...
        const lat = "{{$vessel->location_latitude}}";
        const lng = "{{$vessel->location_longitude}}";
