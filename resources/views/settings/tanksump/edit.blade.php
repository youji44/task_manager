...
@section('content')
                        ...
                        <div class="row align-items-center">
                            <div class="col">
                                <div class="breadcrumbs-area clearfix">
                                    <h4 class="page-title pull-left">Settings > Tank Sump > Edit</h4>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="row">
        <div class="col-md-6 col-sm-12 mt-2">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Edit a Tank Sump</h4>
                    @include('notifications')
                    <form action="{{ route('tf1.settings.tanksump.update') }}" method="POST">
                        @csrf
                        <input hidden name="id" value="{{$tanksump->id}}">
                        <div class="form-group">
                            <label for="tank_no" class="col-form-label">Tank No</label>
                            <input class="form-control" type="text" value="{{$tanksump->tank_no}}" name="tank_no" id="tank_no">
                        </div>
                        <div class="form-group">
                            ...
                            <select id="plocation_id" name="plocation_id" class="custom-select" onchange="select_location(this.value, {{json_encode($locations)}})">
                                @foreach($locations as $item)
                                    <option {{$item->id==$tanksump->plocation_id?'selected':''}} value="{{$item->id}}">{{$item->location}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="location_name" class="col-form-label">Location Name</label>
                            <input class="form-control" value="{{$tanksump->location_name}}" type="text" name="location_name" id="location_name">
                        </div>
                        <div class="form-group">
                            <label for="location_code" class="col-form-label">LOCATION CODE</label>
                            <input class="form-control" value="{{$tanksump->location_code}}" type="text" name="location_code" id="location_code">
                        </div>
                        <div class="form-group">
                            <label for="location_latitude" class="col-form-label">Location Latitude</label>
                            <input class="form-control" value="{{$tanksump->location_latitude}}" type="text" name="location_latitude" id="location_latitude" >
                        </div>
                        <div class="form-group">
                            <label for="location_longitude" class="col-form-label">Location Longitude</label>
                            <input class="form-control" value="{{$tanksump->location_longitude}}" type="text" name="location_longitude" id="location_longitude" >
                        </div>
                        <div class="form-group">
                            <label for="ec_no" class="col-form-label">EC NO#</label>
                            <input class="form-control" value="{{$tanksump->ec_no}}" type="text" name="ec_no" id="ec_no" >
                        </div>
                        <div class="form-group">
                            <label for="storage_product_type" class="col-form-label">Storage Product Type</label>
                            <input class="form-control" value="{{$tanksump->storage_product_type}}" type="text" name="storage_product_type" id="storage_product_type" >
                        </div>
                        <div class="form-group">
